...
In the `require` key of `composer.json` file add `lavary/laravel5-menu": "dev-master`:

```
...
"require": {
	"kiwina/laravel5-menu": "master"
  }  
```

Note that you will also need to add `"illuminate/html": "5.*"` to your composer file in order to use any of the rendering methods such as `asUl()`. Don't forget to add the relevant service provider and facade to your app configuration.
...
At the end of `config/app.php` add `'Menu'    => 'Kiwina\Menu\Facade'` to the `$aliases` array:
...
`laravel5-menu` provides three rendering methods out of the box. However you can create your own rendering method using the right methods and attributes.

As noted earlier, `laravel5-menu` provides three rendering formats out of the box, `asUl()`, `asOl()` and `asDiv()`. You can read about the details [here](#rendering-methods).
...
`laravel5-menu` supports named routes as well:
...
If you don't want to use the routing feature of `laravel5-menu` or you don't want the builder to prefix your URL with anything (your host address for example), you can explicitly set your link's href property:
...
`laravel5-menu` can sort the items based on either a user defined function or a key which can be item properties like id,parent,etc or meta data stored with each item.
...
Config::get('laravel5-menu::views.bootstrap-items') // Laravel 4
config('laravel5-menu.views.bootstrap-items') // Laravel 5
...
@include(Config::get('laravel5-menu::views.bootstrap-items'), array('items' => $mainNav->roots()))
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
      ...
      <ul class="nav navbar-nav navbar-right">
        ...
        @include(Config::get('laravel5-menu::views.bootstrap-items'), array('items' => $loginNav->roots()))
...
*laravel5-menu* is free software distributed under the terms of the MIT license.
